...
### Second local modification: dark/modern UI + activity log

- Reskinned the on-screen controls only (dark background, cards, accent
  color, modern sans-serif type). The print/zine-layout output itself is
  completely unaffected — it always renders on a plain white/grey sheet
  regardless of theme, since that's what needs to be print-accurate.
- Added an "Activity Log" panel near the top that prints a running,
  timestamped log of what the app is doing in the background (PDF loading,
  page counts, blank-page insertion, per-page rendering, layout decisions,
  print triggers). It has Clear/Hide buttons and never appears in the
  printed output.
